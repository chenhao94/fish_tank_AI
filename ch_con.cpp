...
void ch_con::getMap() // Get all infos about the map and the players.
{
...
long ch_con::targetSpeed()
{
	...
	return (long)(max1(ans,1+lev*0.333));
}
...
double ch_con::risk(long x,long y)
{
	double ans=0,rate;
...
double ch_con::benefit(long x,long y)
{
	double ans=0,rate;
	long id,rest,dis;
	for (int i=1;i<=N;i++)
...
void ch_con::run(double riskRate)
{
