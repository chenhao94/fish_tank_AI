...
void player_ch::getMap() // Get all infos about the map and the players.
{
...
bool player_ch::multiHit(long n)
{
	long xx,yy,tx,ty,mx=0,my=0,ax=0,ay=0,max=0,maxexp=-oo;
	long est=0,id,estexp=0,estlev=0;
	for (int i=0;i<=sp;i++)
...
double player_ch::risk(long x,long y)
{
	double ans=0,rate;
...
double player_ch::benefit(long x,long y)
{
	double ans=0,rate;
	long id,rest,dis;
	for (int i=1;i<=N;i++)
...
void player_ch::run(double riskRate)
{
